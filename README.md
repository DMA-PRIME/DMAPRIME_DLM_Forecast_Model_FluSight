...
These are real inconsistencies between the two notebooks. Confirm they match your intent before
trusting cross-notebook results:

1. **`APPLY_SC_EHR_TO_US` differs.** Evaluation sets it to **`False`** (US is selected *without* EHR
   covariates), while submission sets it to **`True`** (US is forecast *with* the SC EHR series). As
   written, the US parameters chosen during evaluation may not match how the US model is used at
   submission time. Set both to the same value for a consistent pipeline.

2. **EHR basename strings differ.** Evaluation looks for `..._dx_cond_lab_Incident*.csv`; submission
   looks for `..._dx_lab_cond_Incident*.csv` (the `cond`/`lab` order is swapped). Make sure both
   point at the file naming your EHR exports actually use, or one notebook will silently fall back to
   zero-filled covariates.

3. **Duplicate DLM structures.** The 4-entry structure grid contains two functional duplicates
   (`level_only` ≡ `level_only_no_seas`, `level_trend` ≡ `level_trend_no_seas`, all with
   `seasonal=None`), so the effective search is over 2 distinct structures.

4. **Quantiles from a Normal approximation.** Intervals come from `μ ± z·SE` on the (log) scale, then
   back-transformed — this is a parametric approximation, not empirically calibrated coverage.

5. **EHR is held flat during forecasting.** Because future EHR is unknown, covariates are carried
   forward at their last observed value, which damps their influence at longer horizons.
